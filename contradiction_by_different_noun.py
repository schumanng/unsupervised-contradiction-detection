...
"""
This script is used to create contradicting hypothesis sentences by changing nouns.
For example, in the sentence "People waiting to get on a train", a contradictory hypothesis sentence can be created by
changing the noun: "People waiting to get on a plane". For this purpose, we have provided all nouns in the sentence with
...
within the sentence (also in terms of tense and numerus). Since the nouns should be as opposite as possible, a threshold
was set for the similarity to the original noun, which had to be undercut for the new noun to be inserted.
...
What you need to do: install required packages and change the path according to your directory (line 254 and 295)
"""
...
# load Spacy-models for POS-Tagging und Dependency-Parsing
...
# load BERT-model for semantic sentence comparision
...
# creates short sentences that focus on the new nouns
...
# exchanges nouns for "MASK-tokens"
def add_masked_labels_for_contradiction (doc):
    ...
    for token in doc:
        if (not mask_added) \
                and (token.pos_ == 'NOUN') \
                and (token.dep_ != 'ROOT' and token.dep_ != 'nsubj'):
...
# predicts masked words (in this case nouns) within a sentence based on the surrounding words
def predict_masked_words_for_contradiction(text, nlp_model, top_k=20):
    ...
    for masked_index in masked_indices:
        ...
        for i, pred_idx in enumerate(top_k_indices):
            ...
            # A new noun is only used if the similarity to the original noun is less than or equal to 0.5
            if (('#' not in predicted_token) and (len(predicted_token) > 4) and (similarity <= 0.5) and (float(token_weight) >= 0.001)):
                token_pos = nlp_model(str(predicted_token))
                if token_pos[0].pos_ == 'NOUN' and (str(predicted_token) not in original_text):
    ...
    if new_modifiers:
        ...
        # get short sentences that focus on the new nouns
        ...
        paraphrased = get_response(new_sent, 4, 4)
...
contradicting_nouns_inserted = []
changes_ct = 0
sent_ct = 0

print('start changing nouns')
for sent in train_df_sample:
    ...
    contradicting_nouns_inserted.append(sent_level_lst)
...
contradicting_nouns_inserted_df = pd.DataFrame(contradicting_nouns_inserted)
counter = 0
for col in contradicting_nouns_inserted_df.columns:
    contradicting_nouns_inserted_df.rename(columns={col:'different_noun_'+str(counter)},inplace=True)
    counter += 1
entirely_different_nouns_df = contradicting_nouns_inserted_df.rename(columns={"different_noun_0": "sent"})

# write final output to disc:
entirely_different_nouns_df.to_csv('output/contradiction_by_different_noun.csv', sep=';')
